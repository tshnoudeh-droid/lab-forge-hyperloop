...
interface FluxHotspotLayerProps {
  activeId: string | null;
  onHotspotClick: (id: string, cameraPreset: [number, number, number]) => void;
}

export default function FluxHotspotLayer({ activeId, onHotspotClick }: FluxHotspotLayerProps) {
  return (
    <>
      {FLUX_HOTSPOTS.map((h) => (
          ...
          <Html center distanceFactor={12} style={{ pointerEvents: "auto" }}>
            <button
              ...
              style={{
                ...
                color: activeId === h.id ? "#0a0a0a" : "#0a0a0a",
